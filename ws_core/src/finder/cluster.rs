...
use cluster::falling_probability::{
    calculate_cumulative_falling_probability_2, calculate_falling_probability_1,
    calculate_falling_probability_2,
};
...
impl Cluster {
    ...
    pub fn header(&self) -> String {
        ...
        let mean_falling_prob1: f32 = self
            .grids
            .iter()
            .map(calculate_falling_probability_1)
            .sum::<f32>()
            / (self.grids.len() as f32)
            * 100.0;
        let mean_falling_prob2: f32 = self
            .grids
            .iter()
            .map(calculate_falling_probability_2)
            .sum::<f32>()
            / (self.grids.len() as f32)
            * 100.0;
        let cumulative_falling_prob: f32 = self
            .grids
            .iter()
            .map(calculate_cumulative_falling_probability_2)
            .sum::<f32>()
            / (self.grids.len() as f32)
            * 100.0;
