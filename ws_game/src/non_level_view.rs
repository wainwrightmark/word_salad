...
impl MavericNode for NonLevelView {
    ...
    fn set_children<R: MavericRoot>(commands: SetChildrenCommands<Self, Self::Context, R>) {
        commands
            ...
            .unordered_children_with_node_and_context(|node, context, commands| {
                let size = &context;

                let text = match node.non_level {
                    NonLevel::BeforeTutorial => "Welcome to Word Salad\nLet's find some Chess Pieces".to_string(),
                    NonLevel::AfterCustomLevel => "Custom Level Complete".to_string(),
                    NonLevel::NoMoreDailyChallenge => "You have completed\nAll daily challenges".to_string(),
                    NonLevel::NoMoreLevelSequence(ls) => format!("You have completed\nAll {}",ls.name()) ,
                };
