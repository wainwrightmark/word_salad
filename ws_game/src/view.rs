...
impl MavericRootChildren for ViewRoot {
    ...
    fn set_children(
        ...
        if context.5.is_closed() {
            ...
            let level = context.1.level(&context.9);

            let close_to_solution = level
                .is_some_and(|level| context.0.is_close_to_a_solution(level, context.2.as_ref()));
            commands.add_child(
                "word_line",
                WordLine {
            ...
            if let Some(text) = TutorialText::try_create(&context.1, &context.2) {
                ...
                if let Some(level) = context.1.level(&context.9){
